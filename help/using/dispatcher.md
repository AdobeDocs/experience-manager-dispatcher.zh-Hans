...
* **更大的处理能力**
在实践中，更大的处理能力意味着Dispatcher在多个AEM实例之间共享文档请求。 由于现在每个实例处理的文档数量减少，您的响应速度将会加快。 Dispatcher 保留每个文档类别的内部统计信息，以便能够估计负载并高效分发查询。

* **更大的故障保护范围**
如果Dispatcher未收到实例的响应，它会自动将请求中继到另一个实例。 如果一个实例变得不可用，唯一的影响就是网站响应变慢，而这与损失的计算能力成比例。 但是，所有服务都继续正常运转。
