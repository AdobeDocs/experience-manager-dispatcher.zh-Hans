...
* DISP-874 -创建 Dispatcher 配置以通过 `DispatcherRestrictUncacheableContent` 标志启用或禁用 DISP-818 的实施。默认值为“禁用”。当设置为“启用”时，系统会删除 mod expires 为不可缓存内容设置的任何缓存标头。这与 4.3.3 版本中的行为（默认值为“启用”）不同，但与 4.3.3 之前的版本（默认值为“禁用”）相同。推荐的方法是保留 `DispatcherRestrictUncacheableContent` 的“禁用”默认值，以便浏览器缓存具有更大的灵活性。从 4.3.3 版升级到 4.3.4 版时，如果要保持与 4.3.3 版相同的行为，则必须将 `DispatcherRestrictUncacheableContent` 明确设置为“启用”。
...
* DISP-698 - Dispatcher TTL 必须支持 s-maxage 和专用指令
* DISP-700 - 模块在 Alpine Linux® 上无法正常工作
* DISP-704 - 包含 %2b 的浏览器请求将发送到未编码的发布者
* DISP-705 - 双重释放或损坏 (fasttop) 导致 Dispatcher 发生崩溃 (`fasttop`)
...
|---|---|---|---|
| Linux® | i686 (32–bit) | 无 | [`dispatcher-apache2.4-linux-i686-4.3.7.tar.gz`](https://download.macromedia.com/dispatcher/download/dispatcher-apache2.4-linux-i686-4.3.7.tar.gz) |
...
| Linux® | x86_64 (64–bit) | 无 | [`dispatcher-apache2.4-linux-x86_64-4.3.7.tar.gz`](https://download.macromedia.com/dispatcher/download/dispatcher-apache2.4-linux-x86_64-4.3.7.tar.gz) |
...
| Linux® | aarch64 (64–bit) | 无 | [`dispatcher-apache2.4-linux-aarch64-4.3.7.tar.gz`](https://download.macromedia.com/dispatcher/download/dispatcher-apache2.4-linux-aarch64-4.3.7.tar.gz) |
...
| macOS | arm64 (64–bit) | 无 | [`dispatcher-apache2.4-darwin-arm64-4.3.7.tar.gz`](https://download.macromedia.com/dispatcher/download/dispatcher-apache2.4-darwin-arm64-4.3.7.tar.gz) |
| macOS | x86_64 (64–bit) | 无 | [`dispatcher-apache2.4-darwin-x86_64-4.3.7.tar.gz`](https://download.macromedia.com/dispatcher/download/dispatcher-apache2.4-darwin-x86_64-4.3.7.tar.gz) |

### IIS {#iis}

| 平台 | 架构 | OpenSSL 支持 | 单击下载 |
|---|---|---|---|
| Windows | x86 (32–bit) | 无 | [`dispatcher-iis-windows-x86-4.3.7.zip`](https://download.macromedia.com/dispatcher/download/dispatcher-iis-windows-x86-4.3.7.zip) |
...
| Windows | x64 (64–bit) | 无 | [`dispatcher-iis-windows-x64-4.3.7.zip`](https://download.macromedia.com/dispatcher/download/dispatcher-iis-windows-x64-4.3.7.zip) |
