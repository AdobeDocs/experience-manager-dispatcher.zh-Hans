...
>[!NOTE]
>
>当Dispatcher前面有一个CDN（或任何其他缓存）时，您应该相应地设置缓存标头，以便CDN不会缓存私有内容。例如： `Header always set Cache-Control private`。
>对于AEM as a Cloud Service，请参阅[缓存](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-cloud-service/content/implementing/content-delivery/caching)页面，了解有关如何设置专用缓存标头的更多详细信息。
